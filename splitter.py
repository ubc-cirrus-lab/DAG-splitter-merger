    ...
    def SuggestSplit(
        ...
        while True:
            ...
            model.objective = xsum(
                [x[i] * splitCandidates[i].GetSplitCommBW() for i in range(len(x))]
            )
            ...
            model.add_constr(
                xsum(
                    [
                        x[i] * splitCandidates[i].GetTotalResources(resourceType="C")
                        for i in range(len(x))
                    ]
                )
                >= r * minDesiredServerPoolReduction["cores"],
                priority=1,
            )
            model.add_constr(
                xsum(
                ...
                >= r * minDesiredServerPoolReduction["mem"],
                priority=1,
            )
            ...
            model.add_constr(
                xsum(
                    [
                        x[i]
                        * splitCandidates[i].GetTotalResources(
                            resourceType="C", objectType="c"
                        )
                        for i in range(len(x))
                    ]
                )
                <= leftCPUPoolCapacity["cores"],
                priority=2,
            )
            model.add_constr(
                xsum(
                    [
                        x[i]
                        * splitCandidates[i].GetTotalResources(
                            resourceType="M", objectType="c"
                        )
                        for i in range(len(x))
                    ]
                )
                <= leftCPUPoolCapacity["mem"],
                priority=2,
            )
            ...
            model.add_constr(
                xsum(
                    [
                        x[i]
                        * splitCandidates[i].GetTotalResources(
                            resourceType="C", objectType="m"
                        )
                        for i in range(len(x))
                    ]
                )
                <= leftMemoryPoolCapacity["cores"],
                priority=2,
            )
            model.add_constr(
                xsum(
                    [
                        x[i]
                        * splitCandidates[i].GetTotalResources(
                            resourceType="M", objectType="m"
                        )
                        for i in range(len(x))
                    ]
                )
                <= leftMemoryPoolCapacity["mem"],
                priority=2,
            )
            ...
            if [x[i].x for i in range(len(x))] == [None for i in range(len(x))]:
                print(
                    "No solution could be found! Easing the minDesiredServerPoolReduction constraint."
                )
