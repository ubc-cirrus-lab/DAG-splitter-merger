...
class Merger:
    def SuggestMerge(self, mergeCandidates, maxServerPoolAllowanceForMerging, serverSize, verbose):
        ...
        for i in range(len(mergeCandidates)):
            model.add_constr( x[i]*mergeCandidates[i].GetTotalResources(resourceType='C') <= serverSize['cores'],
                            priority=1)
            model.add_constr( x[i]*mergeCandidates[i].GetTotalResources(resourceType='M') <= serverSize['mem'],
                            priority=1)
