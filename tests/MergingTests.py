...
class TestMerging(unittest.TestCase):
    ...
    def test_NoMergingPossible1(self):
        ...
        mergingDecisions = self.merger.SuggestMerge(mergeCandidates=[e1, e2, e3],
    ...
    def test_ServerSizeLimits(self):
        e1 = Element(types=['c','m'],
            coreUsageMetric=[1, 0.2], memUsageMetric= [52, 245],
            communicationMetric=[[0, 120], [120,0]])
        e2 = Element(types=['c','c','m'],
            coreUsageMetric=[20, 20, 1], memUsageMetric= [200, 200, 480],
            communicationMetric=[[0, 0, 0], [0, 0, 0], [0, 0, 0]])
        mergingDecisions = self.merger.SuggestMerge(mergeCandidates=[e1, e2],
                                                    maxServerPoolAllowanceForMerging={'cores': 80, 'mem':20480},
                                                    serverSize={'cores': 40, 'mem':10240},
                                                    verbose=False)
